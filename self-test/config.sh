...
function get_trace() {
  # $OUTDIR/traces/$suite/$mutant_trace.csv
  [ "$#" -ne 2 ] && { exception "get_trace()" "Illegal #args"; exit; }
  local crit=$1
  local mut=$2
  echo "$(get_trace_dir $crit)/${mut}_trace.csv"
}
...
function get_coverage_matrix() {
  # $1: coverage criterion {branch, condition, decision, mcdc, random, obc}
  [ "$#" -ne 1 ] \
    && { exception "get_coverage_matrix()" "Illegal #args"; exit; }
  #crit=$1; echo "$INDIR/obligations/${BASENAME}.${crit}_obligations_coverage.csv";
  local crit=$1
  local d="$OUTDIR/coverage_matrices"
  [ ! -d $d ] && mkdir $d
  echo "$d/${crit}.csv"
  #echo "$OUTDIR/obligations/${BASENAME}.${crit}_obligations_coverage.csv";
}
...
function get_mutant_list() {
  ...
  if [ "$#" -ne 1 ]; then
    ...
    crit=$1; base=${BASENAME}
    echo "$OUTDIR/traces/$base.${crit}_tests.csv/$base.${crit}.mutants.csv"
  fi
}
...
function get_test_suite() {
  [ "$#" -ne 1 ] && { exception "get_test_suite()" "Illegal #args"; exit 1; }
  local crit=$1; echo "$OUTDIR/suites/${BASENAME}.${crit}_tests.csv"
}
...
function get_testgen_criteria() {
  [ "$#" -ne 0 ] && { exception "get_criteria()" "Illegal #args"; exit 1; }
  criteria=""
  for cov in ${COVERAGE_CRITERIA[@]}; do
    if [ $cov == "obc" -o $cov == "oobc" -o $cov == "tboobc" \
      -o $cov == "random" -o $cov == "all" ]; then
      continue
    fi
    criteria+=$cov" "
  done
...
function get_cov_dir() {
  if [ "$#" -eq 0 ]; then
    ...
    crit=$1; echo "$OUTDIR/cov/$crit"
  elif [ "$#" -eq 2 ]; then
    crit=$1; bin=$2 echo "$OUTDIR/cov/$crit/$bin"
  else
...
function make_binaries() {
  ...
  echo -e "$HEADER ${BLUE}Copying sources$ENDL"
...
function expand_criteria() {
  [ "$#" -ne 0 ] && { exception "expand_criteria()" "Illegal #args"; exit 1; }
  local covs=""
  for crit in ${COVERAGE_CRITERIA[@]}; do
    if [ $crit == "all" -o $crit == "random" ] ; then
      continue;
    elif [ $crit == "obc" ]; then
      for bin in $(get_binaries); do
        for obc_type in ${OBC_TYPES[@]}; do
          covs+="${crit}_${bin}_${obc_type} "
        done
      done
    elif [ $crit == "oobc" ]; then
      for bin in $(get_binaries); do
