...
std::vector<BPatch_point *> *findEntryPoint(BPatch_addressSpace *app) {
 ...
 appImage->findFunction("testfn", functions);
...
int binaryAnalysis(BPatch_addressSpace *app) {
 ...
 appImage->findFunction("testfn", funcs);
...
void instrumentMemory(BPatch_addressSpace *app) {
 ...
 appImage->findFunction("myprintf", printfFuncs);
 BPatch_funcCallExpr printfCall(*(printfFuncs[0]), printfArgs);

 app->insertSnippet(printfCall, *points);
}
int main() {
 const char *progName = "dyninst-test"; // = ...
 int progPID = 42; // = ...
 const char *progArgv[] = {"dyninst-test", NULL}; // = ...
 // Example 1: create/attach/open a binary
 BPatch_addressSpace *app = startInstrumenting(open,//create, // or attach or open
                                               progName,
                                               progPID,
                                               progArgv);
 ...
 printf("insns_access_memory=%d\n", insns_access_memory);
 // Example 6: instrument memory accesses
 instrumentMemory(app);
 // Example 4: finish up instrumenting
 finishInstrumenting(app, "/export/scratch/vaibhav/dyninst/examples/dyninst-test-1");
 return 0;
}
